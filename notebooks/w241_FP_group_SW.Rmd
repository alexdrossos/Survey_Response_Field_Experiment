...
```
```{r}
table0[, "response_n" :=  paste0("N=", table0[, responsed])]
table0
```

```{r}
# dodgewidth = position_dodge(width=0.9)
# table01 %>% ggplot(aes(x=in_PST, y=response_rate, fill=treatment_group)) +
#   geom_bar(position=dodgewidth, stat='identity') +
#   geom_text(aes(label=response_n, x=in_PST, y=response_rate), position=dodgewidth, vjust=-1) +
#   scale_fill_discrete(name="Treatment Group") +
#   scale_x_discrete(labels=c("False", "True")) +
#   ylim(0,30) +
#   labs(x="Respondent in PST", y = "Response Rate",
#        title= "Response Rate by Treatment Group and Timezone")

dodgewidth = position_dodge(width=0.9)
table0 %>% ggplot(aes(x=cohort_name, y=rr, fill=treatment_group_name)) +
  geom_bar(position=dodgewidth, stat='identity') +
  geom_text(aes(label=response_n, x=cohort_name, y=rr), size=3.5, position=dodgewidth, vjust=-1) +
  ylim(0, 40) +
  scale_fill_discrete(name="Treatment Group") +
  labs(x="Respondent Cohort", y="Response Rate", 
       title = "Response rate by Cohort and Treatment Group")
```
...
```
```{r}
colnames(table01) <- c("treatment_group", 'in_PST', 'count_group', 'response', 'response_rate')
table01[, in_PST := as.factor(in_PST)]
table01[, "response_n" :=  paste0("N=", table01[, response])]
table01[, "response_frac" := paste0(table01[, response],"/",  table01[, count_group])]
```
